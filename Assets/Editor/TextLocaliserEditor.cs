...
namespace Localisation
{
    public class TextLocaliserEditWindow : EditorWindow
    {
        ...
        {
            TextLocaliserEditWindow window = (TextLocaliserEditWindow)CreateInstance(typeof(TextLocaliserEditWindow));
    ...
    public class TextLocaliserSearchWindow : EditorWindow
    {
        ...
        {
            TextLocaliserSearchWindow window = (TextLocaliserSearchWindow)CreateInstance(typeof(TextLocaliserSearchWindow));
        ...
        private void GetSearchResults()
        {
            ...
            foreach (KeyValuePair<string,string> element in dictionary)
            {
                if (element.Key.ToLower().Contains(value.ToLower()) || element.Value.ToLower().Contains(value.ToLower()))
                {
                    ...
                    {
                        if (EditorUtility.DisplayDialog("Remove Key " + element.Key + "?", "This will remove the element from localisation, are you sure?", "Sure, why not?"))
                        {
